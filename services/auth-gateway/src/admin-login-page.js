 ...
 * copy of that open-redirect guard) with '/admin/users' as its fallback
 * instead of sanitizeNext's default '/credentials' — an admin landing here
 * with no explicit `next` (the common case) belongs on the admin panel,
 * not the regular user one. Posts to /admin/login and issues the admin
 * session instead of the regular one.
 * @param {{ next?: unknown, error?: string, username?: unknown }} [options]
 * @returns {string}
 */
export function renderAdminLoginPage({ next, error, username } = {}) {
  const safeNext = sanitizeNext(next, '/admin/users');
