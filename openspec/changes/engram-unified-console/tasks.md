...
- [x] 2.10 Manual E2E, run live on this VPS (2026-09-16) — COMPLETE, one real finding, root-caused and fixed the same day:
  ...
  - [x] **Root cause found and fixed**: that `PATCH` did not reach `cloud_mutations` — confirmed via `engram serve`'s own `GET /sync/status` (curled directly inside the `engram-serve` container, `127.0.0.1:17437`), which reports a structured `phase`/`reason_code`, not just logs. It read `phase: "degraded"`, `reason_code: "non_enrolled_pending_mutations"`: the project (`jagoqui.006-agentes_skills_registry-central-agnt`) had never been through `engram cloud enroll <project>` — autosync correctly refuses to push an unenrolled project's mutations, by design, and was queuing them rather than silently dropping them. `ENGRAM_CLOUD_AUTOSYNC=1`/`ENGRAM_CLOUD_TOKEN`/`ENGRAM_CLOUD_SERVER` were never the issue (confirmed straight from the running child process's `/proc/<pid>/environ`, not just the source). Fixed live: `docker exec ... engram cloud enroll jagoqui.006-agentes_skills_registry-central-agnt` — `/sync/status` went `degraded` → `healthy` within one poll cycle (~30s), and the queued `observation`/`session` upserts landed in Postgres `cloud_mutations` (confirmed by direct query). Any other project synced through this deployment will need the same one-time enrollment — not a per-write action, a per-project one-time step.
...
- [x] 5.10 Manual E2E on this VPS (2026-09-16): a real admin login → Engram Cloud nav link → provisioned on first click, landed inside Cloud's own `/dashboard` with no second login prompt. Confirmed by the user directly ("excelente muy bien"). Multi-admin-distinct-identity not separately re-verified this session (covered by 5.6's automated test).
